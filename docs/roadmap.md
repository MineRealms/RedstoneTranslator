...
| CAD-M0.12.0 | Shared electrical driver query: `world/electrical.rs` used by the simulator and PECA (`fa1d6db`) | 372 non-heavy tests |
| CAD-M0.12.1 | Electrical Legality Filter interface freeze + report-only pre-route pin check (`PinId`/`PinPort`, `NetIndex`, `check_pin_before_route`); 2014 pre-route violations detected before routing | 373 non-heavy tests |
...
| CAD-M0.12.5 | Constraint-directed enumeration (first cut): `DirectOnly` placements outside the source's direct-bound set are skipped before routing; `not_chain` route attempts 1555 -> 71 with 0 failures and an identical final NBT hash; redstone-branch necessary-condition filter added (m06-andnot: 95/2066 failures skipped, hash identical) | 378 non-heavy tests |
